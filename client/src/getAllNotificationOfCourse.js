...
class Happ extends Component {
  constructor(props) {
    ...
    this.state = { data: [], length: [], topic: [], date: [], time: [], room: [], notification: [] };
  ...
  componentDidMount() {
    ...
    axios.get("/course?name=csf210").then(res => {
    ...
    return (
      ...
      <div className="PostNotification">
        <header className="PostNotification-header" style={{ backgroundColor: 'white' }}>
          ...
          <span style={{ color: 'red' }}><h2>All Notifications</h2><br></br></span>
          ...
          {this.state.length<= 0
            ...
            : this.state.length.map(dat => (
                <li style={{ padding: "10px" }} >
                  <span style={{ color: "gray" }}><span style={{ color: "black" }}>topic:</span>  { (this.state.topic)[dat] }</span><br />
                  <span style={{ color: "gray" }}> <span style={{ color: "black" }}>notification:</span> { (this.state.notification)[dat] }</span><br />
                  <span style={{ color: "gray" }}> <span style={{ color: "black" }}>date:</span> { (this.state.time)[dat] }</span><br />
                  <span style={{ color: "gray" }}> <span style={{ color: "black" }}>time:</span> { (this.state.date)[dat] }</span><br /><hr></hr>
