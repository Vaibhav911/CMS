...
class PostNotificationExtraClass extends Component {
  constructor(props)
  {
      ...
      var l='12:00 AM';
     // var d='1212';
      this.state={topic:"Extra class",date:"date",time:l,room:t,notification: "Dear Students,\n "};
  ...
  changeDate(date)
  {
      ...
      this.setState({date:date,notification:" DearStudents,\n There is an Extra Class on "+date+" at "+this.state.time+" in the class "+this.state.room,topic:"Extra class on"+date});
  }
  changeTime(time)
  {
      this.setState({time:time,notification:" DearStudents,\n There is an Extra Class on "+this.state.date+" at "+time+" in the class "+this.state.room});
  }
  changeClass(room)
  {
      //console.lo
      this.setState({room:room,notification:" DearStudents,\n There is an Extra Class on "+this.state.date+" at "+this.state.time+" in the class "+room});
  }
  changeText(e)
  {
    ...
    return (
      <div className="PostNotification">
        <header className="PostNotification-header" style={{backgroundColor:'white'}}>
            <span style={{color:'red'}}><h2>Post Notification For Extra Class</h2></span>
            <span style={{color:'black'}}><span style={{color:'grey'}}>Topic :</span> Extra Class is  on {this.state.date}<br></br></span>
            ...
            <span style={{color:'grey'}}>Resulted Notification:<br></br></span>
